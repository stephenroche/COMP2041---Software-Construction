...
def assessments():
   curr_zid = session['zid']
   grades = {}
   letters = []
   student_details = read_student_details()

   for file in sorted(glob.glob("assessments/" + curr_zid + ".*")):
      marker_zid = re.sub('.*\.', '', file)
      for line in open(file):
         grades[marker_zid] = [line, student_details[marker_zid]['name']]
         letters.append(line)
         break
   ...
   if (length >= 2 and length % 2 == 0 and letters[length//2] != letters[length//2 - 1]):
      median = letters[length//2 - 1] + '/' + letters[length//2]
   else:
      median = letters[length//2]


   return render_template('peer_assessments.html', grades=grades, median=median)
